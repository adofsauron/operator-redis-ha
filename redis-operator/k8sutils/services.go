...
func CheckServiceExist(cr *redisv1alpha1.OperatorRedisHA) (bool, error) {
	logger := serviceLogger(cr.Namespace, cr.ObjectMeta.Name)

	_, err := getService(cr.Namespace, cr.ObjectMeta.Name)
	if err != nil {
		if errors.IsNotFound(err) {
			logger.Info("WARN: CheckServiceExist not found")
			return false, nil
		}

		logger.Error(err, fmt.Sprintf("CheckServiceExist fail, getService err: %v", err))
		return false, err
	}
...
func CreatekService(cr *redisv1alpha1.OperatorRedisHA) error {
	logger := serviceLogger(cr.Namespace, cr.ObjectMeta.Name)

	isServiceExist, err := CheckServiceExist(cr)
	if nil != err {
		logger.Error(err, fmt.Sprintf("CreatekService fail, CheckServiceExist fail, err: %v", err))
		return err
	}
	...
	labels := getRedisLabels(cr.ObjectMeta.Name, cr.ObjectMeta.Labels)
	...
	serviceDef := generateServiceDef(objectMetaInfo, false, ownerDef, true)
